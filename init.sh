...
function init_daloradius {
    ...
    #NEP FIXES FOR FREERADIUS LOGS
    [ -n "$FREERADIUS_LOG_PATH" ] && sed -i "s#\$configValues\['CONFIG_RADIUSLOG_FILE'\] = .*;#\$configValues\['CONFIG_RADIUSLOG_FILE'\] = '$FREERADIUS_LOG_PATH';#" $DALORADIUS_CONF_PATH
    [ -n "$FREERADIUS_SYSLOG_PATH" ] && sed -i "s#\$configValues\['CONFIG_SYSLOG_FILE'\] = .*;#\$configValues\['CONFIG_SYSLOG_FILE'\] = '$FREERADIUS_SYSLOG_PATH';#" $DALORADIUS_CONF_PATH
    [ -n "$FREERADIUS_BOOTLOG_PATH" ] && sed -i "s#\$configValues\['CONFIG_BOOTLOG_FILE'\] = .*;#\$configValues\['CONFIG_BOOTLOG_FILE'\] = '$FREERADIUS_BOOTLOG_PATH';#" $DALORADIUS_CONF_PATH
    ...
    sed -i "s/\$configValues\['CONFIG_LOG_FILE'\] = .*;/\$configValues\['CONFIG_LOG_FILE'\] = '\/data\/log\/daloradius.log';/" $DALORADIUS_CONF_PATH
    chown www-data:www-data $DALORADIUS_CONF_PATH #Added as apparent bugfix
